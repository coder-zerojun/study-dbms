...
SELECT * FROM PLAYER WHERE TEAM_ID = 'K03' AND WEIGHT < 180;

/* PLAYER 테이블에서 TEAM_ID가 'K06'이고 NICKNAME이 제리인 선수 */
SELECT * FROM PLAYER WHERE TEAM_ID = 'K06' AND NICKNAME = '제리';

/* PLAYER 테이블에서 HEIGHT가 170이상이고 WEIGHT가 80이상인 선수 */
...
/* PLATER 테이블에서 NICKNAME이 NULL인 선수 검색 */
...
SELECT PLAYER_NAME, NVL(NICKNAME, '없음') AS 닉네임 FROM PLAYER 
WHERE NICKNAME IS NULL;
...
/* PLAYER_NAME의 포지션은 POSITION 입니다 */
...
SELECT TEAM_NAME 팀이름 FROM TEAM WHERE TEAM_NAME LIKE '%천마'; 
...
/* PLAYER 테이블에서 김씨와 이 씨 찾기 */
SELECT PLAYER_NAME FROM PLAYER WHERE PLAYER_NAME LIKE '김%' OR PLAYER_NAME LIKE '이%';
...
SELECT COUNT(NVL(HEIGHT, '0')) 선수들 FROM PLAYER;
 ...
 * ※ WHERE절에 우선적으로 처리 조건식을 작성해야 속도가 빠르다
...
/* EMPLOYESS 테이블에서 JOB_ID별 평균 SALARY가 10000미만인 JOB_ID 검색, JOB_ID 알파벳순 정렬 */
...
SELECT "POSITION" 포지션, AVG(HEIGHT) "포지션 별 키", (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균 키" FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";
...
SELECT TEL, PLAYER_NAME, T.TEAM_ID 
...
/* JOB_TITLE -> JOBS, EMAIL, LAST_NAME, FIRST_NAME -> EMPLOYEES, */
SELECT J.JOB_TITLE, E.EMAIL, E.LAST_NAME, E.FIRST_NAME 
...
SELECT AVG(E.SALARY) "평균 연봉", JOB_TITLE 
FROM JOBS J JOIN EMPLOYEES E
...
SELECT E.LAST_NAME "사원 성", E.FIRST_NAME "사원 이름"
